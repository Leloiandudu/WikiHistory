...
namespace WikiHistory
{
    ...
    public class ArticleText
  {
    ...
    public int maxLengthOfUnknownSection;
    public string text;
    ...
    public ArticleText(string text)
    {
      ...
      sections.Add(new Section(0, normalizedText.Length,normalizedText));
      getMaxLength();
    }
    ...
    private void getMaxLength()
    {
     ...
     //for (int a = 0; a < sections.Count; a++)
      {
       //   if (sections[a].Length > max) max = sections[a].Length;
          if (s.Length > max) max = s.Length;
      }
      maxLengthOfUnknownSection = max;
    }
    ...
    {
      // now here are some german comments, which I want to translate sometime
      // but now I have to think about the code and not about english words ;)

      string searchtext = rev.fullText;//normalizeString(rev.fullText);
      ...
      int oldPos = 0;
      string sub;
      while ((newlinePos = searchtext.IndexOf("\n\n", newlinePos + 1)) != -1)
      {
        sub = normalizeString(searchtext.Substring(oldPos, newlinePos - oldPos).Trim());
        searchtextLength = Math.Max(searchtextLength, newlinePos - oldPos);
        texts.Add(sub);
        oldPos = newlinePos;
      }
      sub = normalizeString(searchtext.Substring(oldPos, searchtext.Length - oldPos).Trim());
      searchtextLength = Math.Max(searchtextLength, searchtext.Length - oldPos);
      texts.Add(sub);

      // bisher gefundene subsections müssen entfernt werden!
        ...
        foreach(Section s in sections)
      {
           ...
           if(s.revKnown)
        {
          ...
          {
              if (/*(t.Contains(sectionText))&&*/(pos = /*t.IndexOf(sectionText)*/SIndexOf(t,sectionText)) > -1)
            {
      ...
      String ts = "";
      int fqq = 0;
      for (; fqq < texts.Count; fqq++)
      {
          ts = texts[fqq];
          texts[fqq] = ts.Replace("  ", " ");
      }
      fqq = 0;
    for (; fqq < texts.Count; fqq++)
      {
    ...
    for (; fqq < texts.Count; fqq++)
    {
      ...
      // von der gesamtlänge angefangen immer kürzer werden, bis das längste stück gefunden ist!
      string currentSubstring;
      int maxLength = searchtext.Length;
      ...
      //Section ms1=null, ms2=null, ms3=null,ms4=null;

      //max = 0;
      //nrc = 0;
      
     
      if (maxLengthOfUnknownSection < maxLength) maxLength = maxLengthOfUnknownSection;
      //MessageBox.Show(sections.Count + "");
     // if (minLength < 0) { minLength = 1; }
      for (length = maxLength; length >= minLength; length--)
      {
          //if (length % 500 == 0) { MessageBox.Show(length + ""); }
        if ((callingBw != null) && (callingBw.CancellationPending)) return;
        while (callingBw.Paused) { Thread.Sleep(100); }
        // texts.FindAll(delegate(string s) { return s.Length > length; }))
        //foreach (string t in texts) 
    ...
    NewRun:
        //int fq = 0;
        for(int fq=0; fq<texts.Count;fq++){
            t = texts[fq];
       
            for (i = 0; i <= t.Length - length; i++)
          {
              //MessageBox.Show(i + " " + length+" "+t.Length);
              //if (t.Length < length) { continue; }
            currentSubstring = t.Substring(i, length);
            if ((currentSubstring[0] == ' ') && (currentSubstring[currentSubstring.Length - 1] == ' ')
              && (currentSubstring.Trim() != ""))
            {
              ...
              //for(int a=0;a<sections.Count;a++)
               {
                  //s = sections[a];
                 // MessageBox.Show(s.substring);
                   if (!s.revKnown)
                {

                    if (/*s.substring.Contains(currentSubstring) &&*/ ((pos = s.substring.IndexOf(currentSubstring, StringComparison.Ordinal)/*RegexIndexOf(s.substring,currentSubstring)*//*SIndexOf*//*tiof.GetIndex*//*(s.substring,currentSubstring)*/) > -1))
                  {
               //     MessageBox.Show(currentSubstring);
                    // found!
                     // DateTime ts = DateTime.Now;
                    string firstText = t.Substring(0, i).Trim();
                    string lastText = t.Substring(i + length).Trim();
                   ...
                   // MessageBox.Show("FT: " + firstText + " LT: " + lastText);

                    Section firstSection = new Section(s.Start, pos + 1, normalizedText);
                    Section lastSection = new Section(s.Start + pos + length - 1, s.Length - pos - length + 1, normalizedText);
                    Section middleSection = new Section(s.Start + pos + 1, length - 2, rev, normalizedText);

                    sections.Remove(s);
                    //sections.RemoveAt(a);

                    if ((firstSection.Length > 0) && (firstSection.substring.Trim() != "")) sections.Add(firstSection);
                    sections.Add(middleSection);
                    if ((lastSection.Length > 0) && (lastSection.substring.Trim() != "")) sections.Add(lastSection);
